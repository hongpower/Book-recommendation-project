...
# 완제:
key = "EB12AD01EB73638E33B2D89540047BFBE15DF416312A1000D3B1ADF074800676"
# 지윤 :
key = "1D45F65F5DDFAB2120FD1AFF021268E43AE4A2601C4E5F4442E3942A2C149610"
key = "BAA274D1BBCDA95C3AC109F277F2D87778FA4951CC5B8D13108DA841DBE7EB1D"
...
"""
with open('./data/isbn13_rcm_novel.json', 'r') as f:
    isbn13 = json.load(f)['isbn13']

lst = list()

for isbn in isbn13[-1000:]:
    query = isbn
    resp = urllib.request.urlopen(url)
    soup = BeautifulSoup(resp, 'xml')
    try:
        url = soup.find('imageUrl').text
        lst.append([isbn, url])
    except:
        continue
"""