...
def make_json(dic):
    ...
    with open('data/all_novels_data.json', 'w') as f:
        f.write(final_json)

def make_csv(dic):
    with open('data/all_novels_data.csv', 'a', newline='') as f:
        global cols
        dict_writer = csv.DictWriter(f, dic.cols)
...
with open('data/all_novel_ids.json', 'r') as f:
    all_books = json.load(f)['all_books']

with open('data/all_novels_data.json', 'r') as f:
    new_books = json.load(f)['all_books']

cols = pd.read_csv('data/all_novels_data.csv', low_memory=False).columns
