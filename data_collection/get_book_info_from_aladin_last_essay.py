...
def make_json(dic):
    ...
    with open('data/all_essay_data.json', 'w') as f:
        f.write(final_json)

def make_csv(dic):
    global temp_cnt
    temp_cnt += 1
    with open('data/all_essay_data.csv', 'a', newline='') as f:
        global cols
...
with open('data/all_essay_data.json', 'r') as f:
    new_books = json.load(f)['all_books']

cols = pd.read_csv('data/all_essay_data.csv', low_memory=False).columns
