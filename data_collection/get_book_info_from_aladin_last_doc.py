...
def make_json(dic):
    ...
    with open('/book_recommendation_project/data/all_document_data.json', 'w') as f:
        f.write(final_json)

def make_csv(dic):
    global temp_cnt
    temp_cnt += 1
    with open('/book_recommendation_project/data/all_document_data.csv', 'a', newline='') as f:
        global cols
...
with open('data/all_document_ids.json', 'r') as f:
    all_books = json.load(f)['all_books']

with open('/book_recommendation_project/data/all_document_data.json', 'r') as f:
    new_books = json.load(f)['all_books']

cols = pd.read_csv('data/all_document_data.csv', low_memory=False).columns
