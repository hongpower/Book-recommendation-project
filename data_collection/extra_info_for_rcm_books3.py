...
isbn_path = "data_rcm/isbn13_rcm_essay.json"

# 여기 이름 바꿀거면 밑에 저장하는 곳 이름도 바꿔야함!
save_file_name_des = "description_essay"
save_file_name_long = "long_description_essay"
save_file_name_phrase = "phrase_essay"
save_file_name_des_temp = "description_temp_essay"
save_file_name_long_temp = "long_description_temp_essay"
save_file_name_phrase_temp = "phrase_temp_essay"
...
for i in range(len(var_lst)):
    with open(f'{path_to_save}{var_name_lst[i]}_essay.json', 'w') as f:
        f.write(json.dumps(var_lst[i], ensure_ascii=False, indent='\t'))

for i in range(len(var_lst)):
    with open(f'{path_to_save}{var_name_lst[i]}_temp_essay.json', 'w') as f:
        f.write(json.dumps(var_temp_lst[i], ensure_ascii=False, indent='\t'))

# csv :
for i in range(len(csv_lst)):
    with open(f'{path_to_save}{var_name_lst[i]}__essay.csv', 'a') as f:
        tmp_lst = csv_lst[i]
        keys = key_lst[i]
...
for i in range(len(csv_lst)-1):
    with open(f'{path_to_save}{var_name_lst[i]}__temp_novel.csv', 'a') as f:
